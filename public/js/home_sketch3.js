...
function preload() {
  imgs[0] = loadImage("img/MaxHeadshotBW.jpg");
}
...
function nextImage() {
  ...
  if ((width/height) > (targetImg.width/targetImg.height)){
    // 
    targetImg.resize(0,height);
  } else {
    targetImg.resize(width,0);
  }
  img.copy(targetImg, 0, 0, targetImg.width, targetImg.height, int((width-targetImg.width)/2), int((height-targetImg.height)/2), img.width, img.height);
